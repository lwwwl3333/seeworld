...
class EmailController extends Controller {
	...
	public function findpwd() {
		...
		if ($info) {
			...
			$content="亲爱的：".$username."您好，请在下面地址处修改密码，
			请妥善保管好您的密码<br />
			http://".$findpwd."<br />
			密码修改有效期30分钟";
	...
	public function editpwd() {
			...
			$_SESSION ['emailpwd'] = array (
					'key'  => trim($_GET ['key']),   //用户名加密码md5合成
					'info' => trim($_GET ['info']),  //$string 是['username']+$key合成的
					'code' => trim($_GET ['code']),	 //用来检测time时间没有被修改
					'time' => trim($_GET ['time'])	 //当前时间
			);
			...
			if (md5 ( $reinfo ['username'] . '+' . $reinfo ['password'] ) == $_SESSION ['emailpwd'] ['key']) { // 判断URL传输中username是否更改
				...
				if ($_POST ['user_password'] == $_POST ['reuser_password'] && $_POST ['user_password'] != '') {
					...
					if ($edit) {
						...
						$this->success ( '修改成功,请重新登录！！', U('Home/User/login') , 2 );
					} else {
						$this->error ( '修改失败！！' );
					}
				} else {
